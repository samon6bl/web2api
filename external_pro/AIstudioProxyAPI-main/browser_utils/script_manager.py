...
class ScriptManager:
        ...
        try:
            ...
            for model in models:
                ...
                description = model.get(
                    "description", f"Model injected by script {script_version}"
                )
    ...
    async def inject_script_to_page(
        self, page: AsyncPage, script_content: str, script_name: str = "injected_script"
    ) -> bool:
        """将脚本注入到页面中"""
        try:
    ...
    async def setup_model_injection(
        self, page: AsyncPage, script_name: str = "more_models.js"
    ) -> bool:
        """设置模型注入 - 直接注入油猴脚本"""
